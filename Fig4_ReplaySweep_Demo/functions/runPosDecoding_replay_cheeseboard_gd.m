function [res] = runPosDecoding_replay_cheeseboard_gd(p)
%runPosDecoding_replay_cheeseboard_gd, runs Bayesian decoder on a set of
%recordings, adds goal direction to each replay sweep
...
%% theta cycles
chk = D.replayChunks;
chk.tCen = (chk.tEnd - chk.tStart)./2 + chk.tStart;
chk.iCen = round((chk.iEnd - chk.iStart)./2 + chk.iStart);
chk.hd = D.hd(chk.iCen);
chk.hdsm = D.hdsm(chk.iCen);
chk.gd = D.gd(chk.iCen,:);
% chk.rgd = D.rgd(chk.iCen,:);
...
%% get sweep info
sweeps = chunkReplayPosSweeps_wb(D, dec, dec.nspikes);
