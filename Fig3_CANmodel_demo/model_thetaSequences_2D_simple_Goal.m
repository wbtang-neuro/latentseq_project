...
% this is the adapted model, demonstrating goal-directed sweeps
...
x_goal = x_goal.Position; % goal location
colormap('cool')
%% set the model
%parameters
    t_goal = 11215; % time index for goal seeking start 
    ...
    w_FF_goal = 1000;  %amplitude of goal direction exc.
    sig_long = 0.15; %width of goal direction exc.
    sig_short = 0.03; %width of orth goal direction exc.
    
    v_max = 0.4;
    T_theta_max = 0.12;
    [X,Y] = meshgrid((1:n)/n,(1:n)/n); %place field centers

    % apply ramp-like gain to create asymmetric gaussian tunning (an tail towards goal) 
    alpha = 10;  % sharpness control
    gain_scale = 1;
...
%generate rat trajectory as correlated random walk
    ...
    % use previously generated data instead for reproducibility
    x = x_walk;
    ...
    x_goal = (x_goal+0.5)./n; % goal location
...
%% simulation
    for t=1:(T-2)/dt
        ...
        %create goal-direction vector at three pre-defined locations
        if (t >= T/(dt*3) && t < 6872 ) || (t > 8125 && t < 9070) || (t > 9863 && t < 10653) % goal-directional place fields in three predefined locations
            x_curr = x(t,:);
            if ~all(x_curr == x_goal)
                v = x_goal - x(t,:);      % Direction from current to goal
                v = v / norm(v);          % Normalize
                v_orth = [-v(2), v(1)];   % Perpendicular to v
                R = [v; v_orth];          % 2×2 matrix: rows are direction & orthogonal
                D = diag([sig_long^2, sig_short^2]);  % Variances along and orthogonal to v
                Sigma = R' * D * R;                   % Rotate to global coordinates
            end
            g_FF_goal = reshape(mvnpdf([X(:) Y(:)], x_curr, Sigma), n, n); 
            dx = X - x_curr(1);
            dy = Y - x_curr(2);
            dot_to_goal = dx * v(1) + dy * v(2);  % projection toward goal direction
          
            g_FF_goal = g_FF_goal .* (1 + gain_scale * tanh(alpha * dot_to_goal));
            % normalize
            g_FF_goal = w_FF_goal * g_FF_goal / max(g_FF_goal(:));
        else % no goal directional signal at other places
            g_FF_goal = 0;
        end
        %combined input conductance
        G = g_theta_int(t)*(g_rec + g_adapt + g_unifInput + g_feedbackInh + g_FF_goal) + g_theta_FF(t)*g_FF;
        ...
        if mod(t,T_theta_max/dt)==0   
            ...
            hold on, plot(n*x_goal(1)+2.5, n*x_goal(2)+2.5, 'p', 'MarkerSize', 10, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'r'), hold off

            aa = g_rec + g_adapt + g_unifInput + g_feedbackInh + g_FF_goal;
            ...
            ax3 = subplot(3,1,[2 3]); colormap(ax3,'cool');
                ...
                hold on, plot(n*x_goal(1)+2.5, n*x_goal(2)+2.5, 'p', 'MarkerSize', 10, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'r'), hold off
                if t >= T/(dt*3) && t <= (T/(dt*3)+T_theta_max/dt)
                    hold on, text(n/2, n/2, 'goal seeking start','FontSize', 12, ...
                        'Color', 'blue'), hold off
                elseif t >= (t_goal+65) && t <= (t_goal+65+T_theta_max/dt)
                    hold on, text(n/2, n/2, 'goal seeking end','FontSize', 12, ...
                        'Color', 'blue'), hold off
                end
