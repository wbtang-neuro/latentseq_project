...
datasetname = '/Users/wt248/Downloads/LMT-master/simdatadir/simdata3.mat';
...
setopt.niter = 10; % number of iterations
...
%%
load(('2DBump_result.mat'))
figure(2)
xxsampmat = align_xtrue(xxsampmat,xxtrue);
xgrid = gen_grid([min(result_la.xxsampmat(:,1)) max(result_la.xxsampmat(:,1)); min(result_la.xxsampmat(:,2)) max(result_la.xxsampmat(:,2))],50,nf); % x grid for plotting tuning curves
fftc = exp(get_tc(result_la.xxsampmat,result_la.ffmat,xgrid,result_la.rhoff,result_la.lenff));
% xgrid2 = gen_grid([min(xxsampmat(:,1)) max(xxsampmat(:,1)); min(xxsampmat(:,2)) max(xxsampmat(:,2))],50,nf); % x grid for plotting tuning curves
xgrid2 = gen_grid([min(xx(:,1)) max(xx(:,1)); min(xx(:,2)) max(xx(:,2))],50,nf); % x grid for plotting tuning curves

% fftc2 = exp(get_tc(result_la.xxsampmat,ffmat,xgrid,result_la.rhoff,result_la.lenff));
fftc2 = exp(get_tc(xx,ffmat,xgrid2,rhoff,lenff));
fftc_true = simdata.tuningCurve;
...
for ii=1:nneur
    ...
    % xlabel('x');
    % set(gca,'fontsize',15)
    subplot(131),contourf(xg,xg,reshape(fftc2(:,ii),50,[])),title('estimated tc')
    set(gca,'fontsize',15)
    subplot(132),contourf(xg,xg,reshape(fftc(:,ii),50,[])),title('estimated tc')
    set(gca,'fontsize',15)
    subplot(133),contourf(xg,xg,reshape(fftc_true(:,ii),50,[])),title('true tc')
    set(gca,'fontsize',15)
    drawnow,pause
end