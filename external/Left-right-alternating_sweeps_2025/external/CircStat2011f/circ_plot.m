function [a,rad] = circ_plot(alpha, format, formats, varargin)
%
% r = circ_plot(alpha, ...)
...
% % Modified RG to output rmax, the radius of the background plot area
...
switch format
  ...
  case 'hist'
    ...
    if nargin> 3 && varargin{2}
      [~,rmax] = polar_4spoke_halfgrey(t,2*r/sum(r),formats);
      mr = max(2*r/sum(r));
    else
      [~,rmax] = polar_4spoke_halfgrey(t,r,formats);
      mr = max(r);
    end
  ...
  otherwise
    ...
    [~,rmax] = polar_4spoke_halfgrey(alpha, ones(size(alpha)), formats);
end
