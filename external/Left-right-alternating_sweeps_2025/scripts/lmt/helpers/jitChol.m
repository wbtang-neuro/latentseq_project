...
for i = 1:maxTries
  try
    ...
    if jitter == 0;
      ...
      if nargout < 2
        warning('jitChol:addJitter', ['Matrix is not positive definite in jitChol, adding ' num2str(jitter) ' jitter.'])
      end
    ...
    if str2double(verString(1:3)) > 6.1
      [void, errid] = lasterr;
      if any(strcmp(errid, {'MATLAB:posdef', 'parallel:gpu:array:CholPosdef'})) % RJG: added GPU error ID
        nonPosDef = 1;
      end
    else
      errMsg = lasterr;
