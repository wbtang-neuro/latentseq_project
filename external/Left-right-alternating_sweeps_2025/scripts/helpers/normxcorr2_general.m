...
if( m == size(A,1) && n == size(A,2) )
    ...
    s = cumsum(A,1);
    c = [s; s(end,:) - s(1:end-1,:)];
%     c = [s; repmat(s(end,:),m-1,1) - s(1:end-1,:)]; % RJG edit, repmat not needed
    s = cumsum(c,2);
    clear c;
    local_sum_A = [s, s(:,end) - s(:,1:end-1)];
%     local_sum_A = [s, repmat(s(:,end),1,n-1) - s(:,1:end-1)]; % RJG
...
%     i1 = 1:sz(1);
%     i2 = (sz(1)+1) : (2*sz(1)-1);
%     n2 = numel(i1)+numel(i2);
%     c = zeros([n2, sz(2)], "like", A);
%     local_sum_A = zeros([n2, n2], "like", A);
%     
%     s = cumsum(A, 1);
%     c(i1, :) = s;
%     c(i2, :) = s(end,:) - s(1:end-1,:);
%     s = cumsum(c, 2);
%     local_sum_A(:, i1) = s;
%     local_sum_A(:, i2) = s(:,end) - s(:,1:end-1);
%     local_sum_A = cast(local_sum_A, "like", A0);
else
