function [RGB] = mapcolors(x,clims,colmap,n)
%MAPCOLORS maps the data in array x onto a specified colormap.
%   INPUTS
...
% 
% R Gardner 2012-04-11
% Updated 2015-05-29, now works for x of any dimension
...
if nargin<3
    cmap=jet(n);
elseif ischar(colmap) || isstring(colmap)
    cmap = feval(colmap,n);
...
%     if min(x)~=max(x)
        clims=[min(x(:)) max(x(:))];
%     else
%         error('For auto-detecting mapping range, the input x must contain at least two different values')
%     end
